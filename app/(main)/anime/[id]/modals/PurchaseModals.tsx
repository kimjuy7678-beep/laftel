        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/60" onClick={() => { if (!paying) onClose() }}>
            <div className="bg-[#1a1a1a] rounded-2xl w-[380px] border border-white/10 overflow-y-auto max-h-[90vh]" onClick={e => e.stopPropagation()}>
                        ...
                        {showAddCard && (
                            <div className="rounded-xl p-4 border border-white/10 bg-white/[0.03] flex flex-col gap-4">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <input className="w-full bg-transparent border-b border-white/20 focus:border-white/60 outline-none text-sm py-2 text-white placeholder-white/25" value={cardExpiry} onChange={e => setCardExpiry(formatExpiry(e.target.value))} placeholder="MM/YY" maxLength={5} />
                                    <input className="w-full bg-transparent border-b border-white/20 focus:border-white/60 outline-none text-sm py-2 text-white placeholder-white/25" value={cardCvc} onChange={e => setCardCvc(e.target.value.replace(/\D/g, '').slice(0, 4))} placeholder="000" maxLength={4} type="password" />
                                </div>
                    ...
                    <div className="flex flex-col gap-2">
                        ...
                        <div className="grid grid-cols-3 gap-2 mt-2">
                            {[1, 2, 3, 4, 5, 6, 7, 8, 9, '', 0, '⌫'].map((key, i) => (
                                <button key={i} disabled={paying} onClick={() => { if (key === '⌫') setPassword(prev => prev.slice(0, -1)); else if (key !== '' && password.length < 6) setPassword(prev => prev + key) }}
                                    className={`py-3 rounded-xl text-white font-semibold text-lg transition-all ${key === '' ? '' : 'hover:bg-white/10 active:bg-white/20'} disabled:opacity-50`}>{key}</button>
                            ))}
                        </div>
                    </div>
                </div>
                <div className="px-6 pb-6">
                    ...
                    >
                        {paying ? <><div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />결제 중...</> : '결제하기'}
                    </button>
                </div>
            </div>
        </div>
