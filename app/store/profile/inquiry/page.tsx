                    ...
                    {inquiry.status === "답변완료" && inquiry.answer ? (
                        <div className="px-5 py-4 bg-[#f0eeff] border-t border-[#e8e2ff]">
                            <div className="flex items-center gap-2 mb-2">
                                ...
                                {inquiry.answeredAt && (
                                    <p className="ml-auto text-[11px] text-[#c0bcd0]">{inquiry.answeredAt}</p>
                                )}
                            </div>
                            <p className="text-[13px] text-[#3d3755] leading-relaxed whitespace-pre-wrap">{inquiry.answer}</p>
                        </div>
                    ) : (
                        <div className="px-5 py-4 bg-[#fffbf0] border-t border-[#fde68a]/40 flex items-center gap-2">
    ...
    const fetchInquiries = async () => {
        ...
        try {
            ...
            const data = snap.docs.map(d => ({
                ...
                answeredAt: d.data().answeredAt?.toDate?.()?.toLocaleDateString("ko-KR", {
                    year: "numeric", month: "2-digit", day: "2-digit",
                }),
            })) as Inquiry[];
            setInquiries(data);
    ...
    return (
        <>
            ...
            <div className="mb-6 flex items-start justify-between">
                <div>
                    <h2 className="text-[20px] font-bold text-[#16121f]">문의 내역</h2>
            ...
            <div className="mb-5 grid grid-cols-3 gap-3">
                {[
                    ...
                    <div key={s.label} className="rounded-[12px] border border-[#ebe8ff] bg-white px-4 py-3 flex items-center gap-3">
                        <div className="h-9 w-9 shrink-0 rounded-full flex items-center justify-center" style={{ background: s.bg }}>
                            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={s.color} strokeWidth="2">
                                <path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z" />
                            </svg>
                        </div>
                        <div>
                            <p className="text-[11px] text-[#9b94b2]">{s.label}</p>
                            <p className="text-[16px] font-bold" style={{ color: s.color }}>
                                {s.value}<span className="text-[12px] font-medium ml-0.5">{s.unit}</span>
                            </p>
                        </div>
                    </div>
                ))}
            ...
            {loading ? (
                ...
                <div className="flex h-[220px] flex-col items-center justify-center gap-3">
                    ...
                    <p className="text-[13px] text-[#9b94b2]">
                        {tab === "전체" ? "아직 문의 내역이 없어요." : `${tab} 문의가 없어요.`}
                    </p>
                    {tab === "전체" && (
