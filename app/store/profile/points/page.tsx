...
export default function PointsPage() {
    ...
    return (
        <>
            <h2 className="mb-6 text-[20px] font-bold text-[#16121f]">포인트 내역</h2>

            {/* 포인트 요약 카드 */}
            <div className="mb-6 rounded-[16px] bg-[#f0eeff] p-6">
                <div className="flex items-center gap-5">
                    {/* P 아이콘 */}
                    <div className="flex h-[72px] w-[72px] shrink-0 items-center justify-center rounded-full bg-[#7865ff] shadow-[0_6px_20px_rgba(120,101,255,0.35)]">
                        <span className="text-[32px] font-black text-white" style={{ fontStyle: 'italic' }}>P</span>
                    </div>
                    ...
                    {/* 충전 버튼 — OTT 포인트 충전 페이지로 이동 */}
                    <Link
                        href="/point"
                        className="flex h-[38px] items-center gap-1.5 rounded-[10px] bg-[#7865ff] px-5 text-[13px] font-semibold text-white shadow-[0_4px_12px_rgba(120,101,255,0.3)] transition hover:bg-[#6b55f0] active:scale-[0.98]"
                    >
                ...
                <div className="mt-5 grid grid-cols-2 gap-3">
                    <div className="rounded-[12px] bg-white px-4 py-3 flex items-center gap-3">
                        <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-[#f0eeff]">
                            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#7865ff" strokeWidth="2.5">
                    ...
                    <div className="rounded-[12px] bg-white px-4 py-3 flex items-center gap-3">
                        <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-[#fff0f3]">
                            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#ff4d6d" strokeWidth="2.5">
                ...
                {/* OTT 연동 안내 배너 */}
                <Link href="/point"
                    className="mt-4 flex items-center justify-between rounded-[12px] border border-[#c4baff] bg-white px-4 py-3 transition hover:border-[#7865ff] hover:shadow-[0_2px_12px_rgba(120,101,255,0.15)] group">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <p className="text-[12px] font-bold text-[#7865ff]">라프텔 OTT에서 충전하기</p>
                            <p className="text-[11px] text-[#9b94b2]">포인트는 OTT · 스토어 공통으로 사용돼요</p>
                        </div>
                    </div>
            ...
            <div className="rounded-[12px] border border-[#ebe8ff] overflow-hidden">
                {/* 헤더 */}
                <div className="grid grid-cols-[1fr_2fr_auto] gap-4 border-b border-[#ebe8ff] bg-[#faf9ff] px-6 py-3">
                    <p className="text-[12px] font-semibold text-[#9b94b2]">이용날짜</p>
                    <p className="text-[12px] font-semibold text-[#9b94b2]">이용내역</p>
                    <p className="text-[12px] font-semibold text-[#9b94b2] text-right">포인트</p>
                </div>
                ...
                {loading ? (
                    ...
                    paginated.map((r, idx) => (
                        <div key={r.id}
                            className={`grid grid-cols-[1fr_2fr_auto] gap-4 items-center px-6 py-4 transition hover:bg-[#faf9ff] ${idx < paginated.length - 1 ? "border-b border-[#f0edf8]" : ""}`}>
                            <p className="text-[13px] text-[#9b94b2]">{r.date}</p>
                            <div className="flex items-center gap-2">
                                <div className={`h-1.5 w-1.5 shrink-0 rounded-full ${r.type === "earn" ? "bg-[#7865ff]" : "bg-[#ff4d6d]"}`} />
                                <p className="text-[13px] font-medium text-[#16121f]">{r.description}</p>
                                {/* OTT 충전 출처 표시 */}
                                {r.description?.includes("충전") && (
                                    <span className="rounded-[4px] bg-[#f0eeff] px-1.5 py-0.5 text-[10px] font-bold text-[#7865ff]">
                                        OTT
                                    </span>
                                )}
                                {r.description?.includes("스토어") && (
                                    <span className="rounded-[4px] bg-[#fff0f3] px-1.5 py-0.5 text-[10px] font-bold text-[#ff4d6d]">
                                        STORE
                                    </span>
                                )}
                            </div>
                            <p className={`text-[14px] font-bold text-right ${r.type === "earn" ? "text-[#7865ff]" : "text-[#ff4d6d]"}`}>
            ...
            {totalPages > 1 && (
                <div className="mt-8 flex items-center justify-center gap-2">
                    <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1}
                        className="flex h-9 w-9 items-center justify-center rounded-[8px] border border-[#d8d4ee] bg-white text-[#7865ff] transition hover:border-[#7865ff] hover:bg-[#f0eeff] disabled:opacity-30 disabled:cursor-not-allowed">
                        <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M15 18l-6-6 6-6" /></svg>
                    </button>
                    {Array.from({ length: totalPages }, (_, i) => i + 1).map(p => (
                        <button key={p} onClick={() => setPage(p)}
                            className={`flex h-9 w-9 items-center justify-center rounded-[8px] text-[13px] font-medium transition ${p === page ? "bg-[#7865ff] text-white shadow-[0_2px_10px_rgba(120,101,255,0.35)]" : "border border-[#d8d4ee] bg-white text-[#6b647a] hover:border-[#7865ff] hover:bg-[#f0eeff] hover:text-[#7865ff]"}`}>
                            {p}
                        </button>
                    ))}
