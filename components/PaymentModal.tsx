...
import { doc, setDoc, addDoc, collection, getDoc, getDocs, query } from 'firebase/firestore'
...
// 플랜별 멤버십 쿠폰 정의
const MEMBERSHIP_COUPON: Record<PlanId, { label: string; discount: number; type: 'rate' | 'fixed'; maxDiscountAmount?: number }> = {
    anime: { label: '애니 멤버십 스토어 할인 쿠폰', discount: 0.05, type: 'rate', maxDiscountAmount: 2000 },
    allinone: { label: '올인원 멤버십 스토어 할인 쿠폰', discount: 0.1, type: 'rate', maxDiscountAmount: 5000 },
    ost: { label: 'OST 멤버십 스토어 할인 쿠폰', discount: 0.05, type: 'rate', maxDiscountAmount: 1500 },
}
...
function SuccessModal({ plan, onGoHome, onGoHistory }: {
    ...
    useEffect(() => {
        const fire = (particleRatio: number, opts: confetti.Options) => {
            confetti({ origin: { y: 0.7 }, ...opts, particleCount: Math.floor(150 * particleRatio) })
        }
    ...
    return (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
            <div className="relative bg-[#1a1a1a] rounded-2xl w-full max-w-sm p-10 flex flex-col items-center text-center gap-6 border border-white/10">
                ...
                <div className="w-20 h-20 rounded-full overflow-hidden" style={{ border: `2px solid ${plan.color}` }}>
                    <img src="/images/laftel-icon/success-icon.png" alt={plan.name} className="w-full h-full object-cover" />
                </div>

                <div>
                    <h3 className="text-2xl font-black mb-2">구매 완료!</h3>
                    <p className="text-white/50 text-base">멤버십이 시작되었어요 🎉</p>
                    <p className="text-white/40 text-sm mt-1">스토어 할인 쿠폰이 발급되었어요 🎟️</p>
                </div>
...
export default function PaymentModal({ isOpen, onClose, onCloseAll, planId }: PaymentModalProps) {
    ...
    useEffect(() => { if (isOpen && user) loadCards() }, [isOpen, user])

    useEffect(() => {
        if (!isOpen) {
            setAgreed(false); setPayMethod('easy'); setEasyType(null)
            setShowAddCard(false)
            setCardNumber(''); setCardExpiry(''); setCardCvc(''); setCardName('')
            setCardError(''); setShowSuccess(false)
        }
    }, [isOpen])
    ...
    const handleAddCard = async () => {
        ...
        try {
            const uid = (user as any).uid
            const newCard: Card = {
                id: `card_${Date.now()}`, brand: detectBrand(rawNum),
                last4: rawNum.slice(-4), expiry: cardExpiry, isDefault: cards.length === 0,
            }
    ...
    const handlePay = async () => {
        ...
        try {
            ...
            // 1. 이전 멤버십 여부 확인 (첫 가입인지 갱신인지)
            const historySnap = await getDocs(
                query(collection(db, 'users', uid, 'membership_history'))
            )
            const isFirstMembership = historySnap.size === 0
            ...
            await saveNotification(uid, {
                ...
                body: `${plan.name}이 시작되었어요. 즐거운 라프텔 되세요!`,
                link: '/history',
            })

            // 5. 멤버십 스토어 할인 쿠폰 발급 (가입/갱신 모두)
            // issueCoupon 내부에서 쿠폰 알림도 함께 저장됨
            const couponDef = MEMBERSHIP_COUPON[planId]
            await issueCoupon({
                uid,
                ...
                type: couponDef.type,
                minOrderAmount: 5000,
                maxDiscountAmount: couponDef.maxDiscountAmount,
    ...
    const handleGoHome = () => { setShowSuccess(false); onClose(); onCloseAll?.(); router.push('/') }
    const handleGoHistory = () => { setShowSuccess(false); onClose(); onCloseAll?.(); router.push('/history') }

    if (!isOpen) return null

    const easyOptions: { id: EasyType; label: string; bg: string; icon: React.ReactNode }[] = [
        { id: 'toss', label: '토스페이', bg: '#0064FF', icon: <svg width="22" height="22" viewBox="0 0 24 24" fill="white"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-1 14H9V8h2v8zm4 0h-2V8h2v8z" /></svg> },
        { id: 'naverpay', label: '네이버페이', bg: '#03C75A', icon: <span className="text-white font-black text-base">N</span> },
        { id: 'kakaopay', label: '카카오페이', bg: '#FEE500', icon: <span style={{ color: '#3C1E1E' }} className="font-black text-base">K</span> },
        { id: 'phone', label: '휴대폰', bg: '#333', icon: <svg width="20" height="20" viewBox="0 0 24 24" fill="white"><path d="M17 1.01L7 1c-1.1 0-2 .9-2 2v18c0 1.1.9 2 2 2h10c1.1 0 2-.9 2-2V3c0-1.1-.9-1.99-2-1.99zM17 19H7V5h10v14z" /></svg> },
    ]
    ...
    return (
        <>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4"
                    ...
                    <div className="relative bg-[#111] rounded-2xl w-full max-w-2xl max-h-[85vh] overflow-y-auto border border-white/10"
                        ...
                        <div className="flex items-center justify-between px-8 py-6 border-b border-white/10 sticky top-0 bg-[#111] z-10">
                            ...
                            <button onClick={() => { onClose(); onCloseAll?.() }}
                                className="w-9 h-9 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center text-white/60 hover:text-white transition-colors cursor-pointer">✕</button>
                        </div>
                        ...
                        <div className="px-8 py-6 flex flex-col gap-7">
                            ...
                            <div className="rounded-xl p-5 border border-white/10 bg-white/5">
                                ...
                                <p className="text-white/50 text-sm leading-relaxed">{plan.features.join(' · ')}</p>
                                {/* 멤버십 쿠폰 안내 */}
                                <div className="mt-3 flex items-center gap-2 rounded-lg px-3 py-2 text-sm" style={{ background: `${plan.color}15`, border: `1px solid ${plan.color}30` }}>
                                    <span>🎟️</span>
                                    <span style={{ color: plan.color }} className="font-semibold">
                                        가입 시 스토어 {MEMBERSHIP_COUPON[planId].type === 'rate'
                                            ? `${Math.round(MEMBERSHIP_COUPON[planId].discount * 100)}%`
                                            : `${MEMBERSHIP_COUPON[planId].discount.toLocaleString()}원`} 할인 쿠폰 즉시 발급!
                                    </span>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div className="flex gap-3 mb-5">
                                    {[{ id: 'easy' as PayMethod, label: '간편 결제' }, { id: 'card' as PayMethod, label: '카드 결제' }].map(m => (
                                        <button key={m.id} onClick={() => setPayMethod(m.id)}
                                            className="flex items-center gap-2 px-5 py-2.5 rounded-full text-sm font-bold transition-all cursor-pointer"
                                            style={payMethod === m.id ? { background: plan.color, color: '#fff' } : { background: 'rgba(255,255,255,0.08)', color: 'rgba(255,255,255,0.5)' }}>
                                            {m.label}
                                        </button>
                                    ))}
                                </div>
                                ...
                                {payMethod === 'easy' && (
                                    <div className="grid grid-cols-2 gap-3">
                                        {easyOptions.map(opt => (
                                            <button key={opt.id} onClick={() => setEasyType(opt.id)}
                                                className="flex items-center gap-3 px-4 py-3.5 rounded-xl border transition-all cursor-pointer"
                                                style={easyType === opt.id ? { borderColor: plan.color, background: `${plan.color}15` } : { borderColor: 'rgba(255,255,255,0.12)', background: 'transparent' }}>
                                                <div className="w-8 h-8 rounded-lg flex items-center justify-center shrink-0" style={{ background: opt.bg }}>{opt.icon}</div>
                                    ...
                                    <div className="flex flex-col gap-3">
                                        {cards.map(card => (
                                            <button key={card.id} onClick={() => setSelectedCardId(card.id)}
                                                className="flex items-center justify-between px-5 py-4 rounded-xl border transition-all cursor-pointer"
                                                style={selectedCardId === card.id ? { borderColor: plan.color, background: `${plan.color}15` } : { borderColor: 'rgba(255,255,255,0.12)', background: 'transparent' }}>
                                                <div className="flex items-center gap-3">
                                                    <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-white/20 to-white/5 flex items-center justify-center">
                                                        <span className="text-xs font-black text-white/70">{card.brand.slice(0, 2)}</span>
                                                    </div>
                                                    <div className="text-left">
                                                        <p className="text-sm font-bold text-white">{card.brand} •••• {card.last4}</p>
                                                        <p className="text-xs text-white/40">{card.expiry}</p>
                                                    </div>
                                                </div>
                                                {card.isDefault && <span className="text-xs px-2 py-0.5 rounded-full font-bold" style={{ background: `${plan.color}30`, color: plan.color }}>기본</span>}
                                            </button>
                                        ))}
                                        {!showAddCard ? (
                                            <button onClick={() => setShowAddCard(true)}
                                                className="flex items-center gap-2 px-5 py-4 rounded-xl border border-dashed border-white/20 text-white/40 hover:border-white/40 hover:text-white/60 transition-all cursor-pointer">
                                                <span className="text-xl">+</span>
                                                <span className="text-sm font-bold">카드 추가</span>
                                            </button>
                                        ) : (
                                            <div className="flex flex-col gap-4 p-5 rounded-xl border border-white/15 bg-white/5">
                                                <div>
                                                    <p className="text-sm text-white/40 mb-2">카드 번호</p>
                                                    <input className="w-full bg-transparent border-b border-white/20 focus:border-white/60 outline-none text-base py-2 text-white placeholder-white/25 transition-colors"
                                                        value={cardNumber} onChange={e => setCardNumber(formatCardNumber(e.target.value))} placeholder="0000 0000 0000 0000" />
                                                </div>
                                                <div className="grid grid-cols-2 gap-4">
                                                    <div>
                                                        <p className="text-sm text-white/40 mb-2">유효기간</p>
                                                        <input className="w-full bg-transparent border-b border-white/20 focus:border-white/60 outline-none text-base py-2 text-white placeholder-white/25 transition-colors"
                                                            value={cardExpiry} onChange={e => setCardExpiry(formatExpiry(e.target.value))} placeholder="MM/YY" />
                                                    </div>
                                                    <div>
                                                ...
                                                <div className="flex gap-3">
                                                    <button onClick={() => { setShowAddCard(false); setCardError('') }}
                                                        className="flex-1 py-3 rounded-xl border border-white/20 text-white/50 text-base font-bold hover:border-white/40 transition-colors cursor-pointer">취소</button>
                            ...
                            <button onClick={() => setAgreed(!agreed)}
                                ...
                                style={agreed ? { borderColor: plan.color, background: `${plan.color}10` } : { borderColor: 'rgba(255,255,255,0.15)', background: 'transparent' }}>
                                ...
                                <p className="text-base font-bold text-white/70">가격 및 유의사항을 확인하였으며, 매월 정기결제에 동의합니다.</p>
                            </button>

                            {/* 결제 버튼 */}
                            <button onClick={handlePay} disabled={!isPayable() || loading}
            ...
            {showSuccess && <SuccessModal plan={plan} onGoHome={handleGoHome} onGoHistory={handleGoHistory} />}
        </>
    )
}